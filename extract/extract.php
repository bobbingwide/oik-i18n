...
class StringExtractor {
	...
	function extract_from_directory( $dir, $excludes = array(), $includes = array(), $prefix = '' ) {
		...
		foreach ( $file_names as $file_name ) {
			...
			if ( preg_match( '/\.(php|inc)$/', $file_name ) && $this->does_file_name_match( $prefix . $file_name, $excludes, $includes ) ) {
			  $extract_from_file =  $this->extract_from_file( $file_name, $prefix );
				$translations->merge_originals_with( $extract_from_file );
			}
		...
		//	}
			...
			if ( is_dir( $file_name ) ) {
			  $prefix_file_name_slash =  $prefix . $file_name . '/';
				$extract_from_directory = $this->extract_from_directory( $file_name, $excludes, $includes, $prefix_file_name_slash );
				$translations->merge_originals_with( $extract_from_directory );
			}
		}
		chdir( $old_cwd );
		return $translations;
