...
require_once dirname( __FILE__ ) . '/oik-ext-meta.php';
...
/**
 * Add the rules for the i18n oik APIs
 *
 * Note: Certain strings are not translatable e.g. WordPress, Drupal
 * Certain functions have parameters that are string literals but should not be translated
 * - bw_trace()
 * - bw_trace2() 
 * since these are used for problem determination, which is assumed to be performed in English
 *
 * Some APIs can receive variables rather than translated strings.
 * Others receive variables which are not translated until required
 *
 * oik uses bw_translate() instead of __() in functions which actually perform the string translation
 * bw_translate() is different from __() in that the text domain is accessed using bw_context()
 * This might help reduce the code complexity. Fingers crossed eh? 

 *
 */
function add_oik_rules( $rules ) {
  $rules['bw_translate'] = array( 'string' );
  $rules['_bwtnt'] = array( 'string', null );
  $rules['bw_array_get_dcb'] = array( null, null, 'string' );
  $rules['bw_textfield'] = array( null, null, 'string', null, null, null );
  $rules['bw_emailfield'] = array( null, null, 'string', null, null, null );
  $rules['bw_textarea'] = array( null, null, 'string', null, null ); 
  $rules['bw_radio'] = array( null, null, 'string', null, null, null ); 
  // $rules['label']
  // submit
  // 
  $rules['bw_textfield_arr'] = array( null, 'string', null, null, null, null, null );
  $rules['bw_emailfield_arr'] = array( null, 'string', null, null, null, null, null );
  $rules['bw_textarea_arr'] = array( null, 'string', null, null, null, null, null );
  $rules['bw_select_arr'] = array( null, 'string', null, null, null, null, null );
  ...
  //$rules['h4'] = array( 'string' );
  //$rules['h5'] = array( 'string' );
  //$rules['h6'] = array( 'string' );
  $rules['li'] = array( 'string' );
  $rules['lit'] = array( 'string' );
  $rules['bwt'] = array( 'string' );
  $rules['bw_form_field'] = array( null, null, 'string' );
  $rules['alink'] = array( null, null, 'string', 'string', null, null );
  $rules['_alink'] = array( null, null,'string', 'string' );
  $rules['th'] = array( 'string' );
...
class MakePOT {
	...
	function wp_generic($dir, $args) {
                echo "Processing wp_generic" . PHP_EOL;
		$defaults = array(
			'project' => 'wp-core',
			'output' => null,
	...
	function wp_core($dir, $output) {
		if ( file_exists( "$dir/wp-admin/user/about.php" ) )  { gobang(); return false; }
...
/**
 * Run the makeoik process
 *
 * @param array $argv parameters to makeoik.php
 *
 */  
function runmakeoik( $argv ) {
	$makepot = new MakePOT;
